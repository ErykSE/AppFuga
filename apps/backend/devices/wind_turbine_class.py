...
class WindTurbine(EnergySource):
    """
    Klasa dla urządzeń generujących moc typu Wind Turbine (turbina wiatrowa).
    ...
    def __init__(
        ...
        super().__init__(
            ...
            is_adjustable=True,  # Turbina wiatrowa jest regulowana
    ...
    def from_dict(cls, data, info_logger=None, error_logger=None):
        """Tworzy instancję WindTurbine ze słownika (z JSON API)."""
        return cls(
            id=data.get("id"),
            name=data.get("name"),
            priority=data.get("priority", 2),  # Domyślny priorytet 2
