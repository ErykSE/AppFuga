...
class FuelTurbine(EnergySource):
    """
    Klasa dla urządzeń generujących moc typu Fuel Turbine (turbina paliwowa).
    ...
    def __init__(
        ...
        super().__init__(
            ...
            is_adjustable=True,  # Turbina paliwowa jest regulowana
    ...
    def from_dict(cls, data, info_logger=None, error_logger=None):
        """Tworzy instancję FuelTurbine ze słownika (z JSON API)."""
        return cls(
            id=data.get("id"),
            name=data.get("name"),
            priority=data.get("priority", 3),  # Domyślny priorytet 3
